...
def test_sync_records_client_version_for_admin_users(client):
    ...
    sync_response = client.get(
        ...
        headers={
            **auth(admin_token),
            ...
            "X-Client-Version": "1.4.5",
            "X-Client-Version-Code": "28",
    ...
    assert user["last_client_version"] == "1.4.5"
    assert user["last_client_version_code"] == 28
...
def test_metrics_contains_counts_and_no_email(client):
    ...
    assert response.status_code == 200
    assert response.json()["version"] == "1.4.5"
